...
export const fetchImageFromDirectus = async (id, key = 'bild-small') => {
    const baseURL = 'https://directus.uxulmassgrave.uni-bonn.de/assets/';
    const fullURL = `${baseURL}${id}?key=${key}`;

    console.log('Fetching image from URL:', fullURL); // Add this line to log the URL

    try {
        const response = await fetch(fullURL, {
            headers: { Authorization: `Bearer ${VITE_APP_ACCESS_TOKEN}` },
        });
        if (!response.ok) {
        ...
        return null; // Return null or a default image URL in case of error
    }
};
...
export async function fetchPosts(recordId, languageCode = selectedLanguage.value.lang) {
    try {
        ...
        if (recordData) {
            ...
            console.error('records_by_id not found in response:', response);
