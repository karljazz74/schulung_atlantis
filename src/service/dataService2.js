...
const VUE_APP_ACCESS_TOKEN = 'c3NgZFYeNCCfIFv8rpQ4RFHb9-gCRa8t';
const VUE_APP_GRAPHQL_ENDPOINT = 'https://directus.uxulmassgrave.uni-bonn.de/graphql';

export const client = createDirectus(VUE_APP_GRAPHQL_ENDPOINT).with(graphql());
console.log('Directus client initialized:', client);

// Add your fetchImageFromDirectus function here
export const fetchImageFromDirectus = (id, key = 'bild-med') => {
    ...
    try {
        const query = `query {
            records(limit: 500) {
                  ...
                  find_location
                  lote_number
                  translations_records(filter: { languages_code: { code: { _eq: "de-DE" } } }) { 
                      languages_code {
                          code
                      }
                      tr_description
                  ...
                  fk_category {
                      TranslationCat: translations(filter: { languages_code: { code: { _eq: "de-DE" } } }) {
                          obj_categories
                          obj_description
        ...
        const response = await fetch(VUE_APP_GRAPHQL_ENDPOINT, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${VUE_APP_ACCESS_TOKEN}`,
            },
            body: JSON.stringify({ query }),
        });
        console.log('Token:', VUE_APP_ACCESS_TOKEN);
